...
def run_full_analysis(df, reg_len=20, n1=10, n2=21, obLevel1=80, obLevel2=40, osLevel1=-80, osLevel2=-40):
    ...
    log_ret_5  = np.log(df['Close'] / df['Close'].shift(5).replace(0, np.nan))
    atr_14_v4  = tr.rolling(14).mean()
    atr_pct_v4 = (atr_14_v4 / df['Close']).replace(0, np.nan)
    ...
    price_dir_5  = np.sign(df['Close'].diff(5))
    ...
    if st.session_state["results_df"] is None and st.session_state.get("corr_data") is None and not st.session_state.get("run_screener_flag") and not st.session_state.get("timeseries_done"):
        ...
        if st.session_state["results_df"] is not None and not st.session_state.get("timeseries_done"):
            ...
            with tab_raw:
                ...
                display_df = results_df.sort_values("Priority_Long", ascending=False)[cols]
                ...
                # --- Export Logic with Column Filtering ---
                # We filter out internal timing dots and metadata for a cleaner Excel report
                def get_clean_df(df_input):
                    drop_patterns = ['_Today', '_1d', '_2d', '_3d', '_5d']
                    cols_to_drop = [c for c in df_input.columns if any(p in c for p in drop_patterns)]
                    # Also drop internal tie-breakers (starting with _) and UI/Color fields
                    cols_to_drop += [c for c in df_input.columns if c.startswith('_')]
                    cols_to_drop += [c for c in ['Narrative_Color', 'SimpleName', 'DisplayName', 'WT1_5ago'] if c in df_input.columns]
                    return df_input.drop(columns=cols_to_drop)
                ...
                with dl_col1:
                    st.download_button(
                        label="↑  Bullish Signals (Excel)",
                        data=to_excel(get_clean_df(results_df[results_df['Signal'] > 0])),
                        file_name=f"bullish_signals_{analysis_date}.xlsx",
                        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                        key="dl_bullish_timing",
                        help="All active bullish signals (Cleaned Quant Data)",
                        use_container_width=True
                    )
                with dl_col2:
                    st.download_button(
                        label="↓  Bearish Signals (Excel)",
                        data=to_excel(get_clean_df(results_df[results_df['Signal'] < 0])),
                        file_name=f"bearish_signals_{analysis_date}.xlsx",
                        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                        key="dl_bearish_timing",
                        help="All active bearish signals (Cleaned Quant Data)",
                        use_container_width=True
                    )
                ...
                with dl_col3:
                    st.download_button(
                        label="↑  Top 10 Bullish (Excel)",
                        data=to_excel(get_clean_df(top_longs)),
                        file_name=f"top10_bullish_{analysis_date}.xlsx",
                        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                        key="dl_top10_bullish",
                        help="Top 10 cleaned bullish signals ranked by Pulse",
                        use_container_width=True
                    )
                with dl_col4:
                    st.download_button(
                        label="↓  Top 10 Bearish (Excel)",
                        data=to_excel(get_clean_df(top_shorts)),
                        file_name=f"top10_bearish_{analysis_date}.xlsx",
                        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                        key="dl_top10_bearish",
                        help="Top 10 cleaned bearish signals ranked by Pulse",
                        use_container_width=True
                    )
